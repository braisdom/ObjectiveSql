    ...
    private Object createBean(ResultSet rs) throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            if (fieldName != null) {
                ...
                if (!fieldType.equals(rawColumnValue.getClass()))
                    throw new ClassCastException(String.format("Inconsistent data types " +
                                    "between %s and %s, %s vs %s", fieldName, columnName, fieldType.getName(),
                            rawColumnValue.getClass().getName()));
